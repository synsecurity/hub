...
init_tests() {
    ...
    then
       BRANCH=$1
       git clone --branch $BRANCH https://github.com/synsecurity/hub-tests.git 
       cd hub-tests/ && go get -u bitbucket.org/Aishee/synsec@$BRANCH || true &&  go build && cd ..
    else
       git clone  https://github.com/synsecurity/hub-tests.git
